...
void Main_Joueur::set_carte_empty(int* position_tab ){
    for (int i(0); i<sizeof(*position_tab/sizeof(position_tab[0])); i++) {
        m_carte_en_main[position_tab[i]].set_empty(0);
    }
};
...
void Main_Joueur:: print_carte_en_main(){
    for (int i(0); i<13; i++) {
        if (m_carte_en_main[i].get_empty()!=0) {
...
int Main_Joueur::test_postion_existed(string position){
    int position_int(0);
    position_int=atoi(position.c_str());
    while (test_position_int(position) || (position_int<0) || (position_int > 12) || (m_carte_en_main[position_int].get_empty())==0 ) {
...
int* Main_Joueur::attack_number_carte(int number_of_carte){
    print_carte_en_main();
    ...
    for (int i(0); i<number_of_carte;i++) {
        ...
        string position("13");
        getline(cin, position);
        position_tab[i]=test_postion_existed(position);
    }
    return position_tab;
};
void Main_Joueur::set_carte_to_carte_to_fight(int position_tab[], Carte* carte_to_fight, int number_carte){
...
void Main_Joueur::attack_plural(Carte_cible carte_cible_to_fight){
    ...
    int* position_tab = attack_number_carte(number_of_carte);
    while (!test_chain_carte(number_of_carte, position_tab)) {
        position_tab = attack_number_carte(number_of_carte);
    }
    int position_max_value_carte_en_main(0);
    Value_of_Carte max_value_carte_en_main=m_carte_en_main[position_tab[0]].get_value_carte();
    for (int i(1); i<number_of_carte; i++) {
        if (max_value_carte_en_main < m_carte_en_main[position_tab[i]].get_value_carte()) {
            max_value_carte_en_main=m_carte_en_main[position_tab[i]].get_value_carte();
